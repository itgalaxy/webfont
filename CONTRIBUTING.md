...
- **Never use `secrets` in `jobs.<id>.if`.** The `secrets` context is not available there; GitHub fails to parse the workflow (`Unrecognized named-value: 'secrets'`) and **`workflow_dispatch` returns HTTP 422** — which blocks Release Please from starting [`npm-publish.yml`](.github/workflows/npm-publish.yml). To skip an optional job when a secret is unset, map it to job `env` and gate individual steps (or a shell gate that sets an output), as in `bump-homebrew-core`.
