...
| What changed | Update |
|--------------|--------|
| CLI flags, aliases, help text, or exit codes | Edit `packages/webfont/src/cli/meow/cliFlagCatalog.ts`, run `npm run docs:cli` → [packages/webfont/docs/cli.md](./packages/webfont/docs/cli.md); update [FEATURES.md](./FEATURES.md) when capability changes |
...
| `webfont()` options, defaults, return shape, or supported inputs/outputs | [packages/webfont/docs/configuration.md](./packages/webfont/docs/configuration.md), [README.md](./README.md) Capabilities at a glance, [packages/webfont/README.md](./packages/webfont/README.md), [FEATURES.md](./FEATURES.md) |
...
- **PR preview installs ([pkg.pr.new](https://pkg.pr.new)).** Open pull requests publish a preview `webfont` build when CI passes on the Node **24.x** matrix leg ([#787](https://github.com/itgalaxy/webfont/issues/787)). The bot comment (updated on each push via `--comment=update`) includes install URLs — use those exact links from the PR you are testing:
  - **npm dependency:** `npm install --save-dev <pkg.pr.new URL from comment>`
  - **CLI (no install):** `npx <pkg.pr.new URL from comment> --version`
  Previews are **not** on the public npm registry and do not replace a release from `master`. Repository maintainers must install the [pkg-pr-new GitHub App](https://github.com/apps/pkg-pr-new) before comments appear.
